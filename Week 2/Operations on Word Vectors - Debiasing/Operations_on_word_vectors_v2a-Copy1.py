...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = np.dot(np.divide(np.dot(e,g),np.sum(np.dot(g,g))),g)
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu = np.divide(np.sum(e_w1,e_w2),2)

    # Step 3: Compute the projections of mu over the bias axis and the orthogonal axis (≈ 2 lines)
    mu_B = np.dot(np.divide(np.dot(mu,bias_axis),np.sum(np.dot(bias_axis,bias_axis))),bias_axis)
    mu_orth = mu-mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.dot(np.divide(np.dot(e_w1,bias_axis),np.sum(np.dot(bias_axis,bias_axis))),bias_axis)
    e_w2B = np.dot(np.divide(np.dot(e_w2,bias_axis),np.sum(np.dot(bias_axis,bias_axis))),bias_axis)
